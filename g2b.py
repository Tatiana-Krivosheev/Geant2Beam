...
def main():
    ...
    if len(sys.argv) < 3:
        print("g2b input_fname output_phsf_name_without_extension <optional # of original events>")
        return

    nof_original = 10000000000
    if len(sys.argv) > 3:
        nof_original = int(sys.argv[3])

    events = load_events(sys.argv[1])

    header = make_header( nof_original, events)
    (mode, NPPHSP, NPHOTPHSP, EKMAX, EKMIN, NINCP) = header
